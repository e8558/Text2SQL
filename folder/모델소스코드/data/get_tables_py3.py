...
def dump_db_json_schema(db, f):
    ...
    cursor = conn.execute("SELECT name FROM sqlite_master WHERE type='table' or type='view';")
...
if __name__ == '__main__':
    ...
    for f, df in db_files:
        ...
        """
            if you want get all of db_files
        """
        if not GET_ALL_DB and df in ex_tabs.keys():
    ...
    with open(output_file, 'wt') as out:
        ...
        json.dump(tables, out, sort_keys=True, indent=4, separators=(',', ': '), ensure_ascii=False)