...
def eval_where(pred, label):
    ...
    for unit in pred_conds:
        if unit in label_conds:
            cnt += 1
            # print("label units -", label_conds)
            # print("target unit -", unit)
            # print()
            label_conds.remove(unit)
        if unit[2] in label_wo_agg:
            cnt_wo_agg += 1
            # print("22 label units -", label_wo_agg)
            # print("22 target unit -", unit[2])
            # print()
...
class Evaluator:
    ...
    def eval_hardness_details(self, sql):
        count_comp1_ = count_component1(sql)
        count_comp2_ = count_component2(sql)
        count_others_ = count_others(sql)

        if count_comp1_ <= 1 and count_others_ == 0 and count_comp2_ == 0:
            return (count_comp1_, count_comp2_, count_others_)
        elif (count_others_ <= 2 and count_comp1_ <= 1 and count_comp2_ == 0) or (
            count_comp1_ <= 2 and count_others_ < 2 and count_comp2_ == 0
        ):
            return (count_comp1_, count_comp2_, count_others_)
...
def print_scores(scores, etype):
    ...
    if SHOW_ORIGIN:
        print("\n\n{:20} {:20} {:20} {:20} {:20} {:20}".format("", *levels))
        ...
        if etype in ["all", "match"]:
            # print("\n====================== Logical-Form Accuracy without value =====================")
            exact_scores = [scores[level]["exact"] * 100 for level in levels]
            print(
                "{:20} {:<20.1f} {:<20.1f} {:<20.1f} {:<20.1f} {:<20.1f}".format(
                    "lf (without value)", *exact_scores
                )
            )
        ...
        if etype in ["all", "match"]:
            print("\n---------- without value ----------")
            for type_ in partial_types:
                this_scores = [scores[level]["partial"][type_]["acc"] * 100 for level in levels]
                print("{:20} {:<20.1f}".format(type_, this_scores[-1]))

            exact_scores = [scores[level]["exact"] * 100 for level in levels]
            print("\nlf_acc:", round(exact_scores[-1], 1))
...
def evaluate(gold, predict, db_dir, etype, kmaps):
    ...
    eval_err_num = 0
    # i = 0
    # dev_list = [(p, g) for p, g in zip(plist, glist)]
    # for p, g in tqdm(dev_list):
